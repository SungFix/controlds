...
async function v46PickupRpc(args={}){if(!navigator.onLine){setSyncState("error");throw new Error("offline");}setSyncState("saving");const{data:result,error}=await sb.rpc("ete_pickup_request_v2",args);if(error){setSyncState("error");throw error;}if(!result?.ok){setSyncState("online");throw new Error(String(result?.error||"pickup_failed"));}await v46LoadData(false);return result.request;}
