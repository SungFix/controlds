...
function v46MapHistory(row){const item=normalizeHistoryItem({id:row.id,atISO:row.at_iso,at:row.at_iso?new Date(row.at_iso).toLocaleTimeString("pt-BR",{timeZone:"America/Recife",hour:"2-digit",minute:"2-digit"}):recifeTimeLabel(),text:row.text,detail:row.detail||"",type:row.type||"system",responsible:row.responsible||"Sistema"});item.studentId=row.student_id||null;item.requestId=row.request_id||null;item.permissionId=row.permission_id||null;item.notebookId=row.notebook_id||null;return item;}
...
async function v46RefreshAfterWrite(){try{await v46LoadData(false);return true;}catch(err){console.error("Alteração salva, mas a atualização da interface falhou:",err);v46ScheduleReload();return false;}}
async function v46Rpc(name,args={}){if(!navigator.onLine){setSyncState("error");throw new Error("offline");}setSyncState("saving");const{data:result,error}=await sb.rpc(name,args);if(error){setSyncState("error");throw error;}await v46RefreshAfterWrite();return result;}
async function v46PickupRpc(args={}){if(!navigator.onLine){setSyncState("error");throw new Error("offline");}setSyncState("saving");const{data:result,error}=await sb.rpc("ete_pickup_request_v2",args);if(error){setSyncState("error");throw error;}if(!result?.ok){setSyncState("online");throw new Error(String(result?.error||"pickup_failed"));}await v46RefreshAfterWrite();return result.request;}
