(function initCentralPortal(){
  ...
  const systems=[
    ...
    {id:"atestados",name:"Atestados",description:"Área separada para o futuro sistema de atestados.",icon:"▤",status:"Em preparação"}
  ];
  ...
  function buildExitLayer(){
    ...
    exitLayer.addEventListener("click",function(event){
      ...
      if(event.target.closest("[data-exit-account]")){
        closeExitMenu();
        try{const result=logoutApp?.();if(result&&typeof result.catch==="function")result.catch(function(err){console.error("Falha ao sair:",err);});}
  ...
  function openExitMenu(anchor){
    ...
    requestAnimationFrame(function(){positionExitMenu(exitAnchor);});
  }
  ...
  function buildPortal(){
    ...
    const cards=systems.map(function(system,index){const availability=index===0?"available":"preparing";return '<button class="ete-system-card '+availability+'" type="button" data-system="'+system.id+'"><span class="ete-system-card-head"><span class="ete-system-icon" aria-hidden="true">'+system.icon+'</span><span class="ete-system-status">'+system.status+'</span></span><span class="ete-system-copy"><strong>'+system.name+'</strong><p>'+system.description+'</p></span><span class="ete-system-open"><span>'+(index===0?'Acessar sistema':'Ver módulo')+'</span><span aria-hidden="true">→</span></span></button>';}).join("");
    portal.innerHTML=''
      +'<div class="ete-portal-shell"><header class="ete-portal-topbar">'
      ...
      +'<div class="ete-portal-actions"><span class="ete-portal-user" id="etePortalUser"></span><button class="ete-portal-theme" type="button" id="etePortalTheme">Tema</button><button class="ete-portal-exit" type="button" id="etePortalExit" aria-haspopup="menu" aria-expanded="false">Sair</button></div>'
      +'</header><main class="ete-portal-home">'
      ...
      +'<div class="ete-portal-section-head"><span>Seus sistemas</span><small>'+systems.length+' módulos cadastrados</small></div><div class="ete-portal-grid">'+cards+'</div></main>'
      +'<main class="ete-portal-module"><button class="ete-portal-back" type="button" id="etePortalBack">← Voltar ao portal</button><section class="ete-module-panel"><span class="ete-portal-kicker">Módulo separado</span><h1>Atestados</h1><p>Esta área está isolada do Control Ds e pronta para receber as telas e funções próprias do sistema de atestados.</p><div class="ete-module-note">O Control Ds continua preservado. Novos módulos podem ser adicionados sem misturar dados, permissões ou funcionalidades.</div></section></main></div>';
  ...
  function showPortalHome(){if(!isAuthenticated())return;closeExitMenu();buildPortal();portal.classList.remove("module-open");refreshUser();portal.hidden=false;document.body.classList.add("portal-open");setSelectedSystem("");}
  function hidePortal(){if(!portal)return;closeExitMenu();portal.hidden=true;portal.classList.remove("module-open");document.body.classList.remove("portal-open");}
  function openSystem(systemId){if(systemId==="control-ds"){setSelectedSystem("control-ds");hidePortal();return;}if(systemId==="atestados"){setSelectedSystem("atestados");buildPortal();portal.classList.add("module-open");refreshUser();portal.hidden=false;document.body.classList.add("portal-open");}}
  ...
  function installExitHook(){
    ...
    document.addEventListener("click",function(event){
      ...
      if(button&&isAuthenticated()){
        ...
        if(exitLayer&&!exitLayer.hidden&&exitAnchor===button)closeExitMenu();else openExitMenu(button);
        return;
      }
      if(exitLayer&&!exitLayer.hidden&&!event.target.closest?.(".ete-exit-card")&&!event.target.closest?.("#etePortalExit"))closeExitMenu();
    },true);
    document.addEventListener("keydown",function(event){if(event.key==="Escape"&&exitLayer&&!exitLayer.hidden)closeExitMenu();});
    window.addEventListener("resize",function(){if(exitLayer&&!exitLayer.hidden)positionExitMenu(exitAnchor);});
    window.addEventListener("scroll",function(){if(exitLayer&&!exitLayer.hidden)positionExitMenu(exitAnchor);},true);
  }
